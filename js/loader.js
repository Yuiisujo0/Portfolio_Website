// js/loader.js
// Your loader logic with main-content integration removed.
// Assumes THREE and gsap are globally available (loaded before this file).
...
(function () {
  ...
  if (!loaderEl || !counterEl || !canvasContainer) {
    // If any required element is missing, nothing to run.
    return;
  }
  ...
  function exitLoaderSequence() {
    // Simplified exit: animate spheres away, fade out loader, then remove it.
    ...
    tl.to(loaderEl, {
      ...
      onComplete: () => {
        ...
        try { loaderEl.remove(); } catch (e) { /* ignore */ }
        // stop animation loop and dispose renderer to free GPU memory
  ...
  function createSingleWrapTexture(text, opts = {}) {
    ...
    const appliedScale = hScale * squeezeFactor;
  ...
  function createRepeatTextTexture(text, opts = {}) {
    ...
    if (rows === 1) {
      ...
      for (let r = 0; r < total; r++) rowYs.push(start + r * spacingY);
    }
  ...
  // Safety timeout if loader never completes
