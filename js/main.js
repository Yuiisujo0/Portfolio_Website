...
(function () {
  ...
  function init() {
    ...
    /* ---------- Important change ----------
       Do NOT set the initial cover transform immediately here.
       Instead set it only after we have a reliable image size (inside setInitialCover).
       This prevents incorrect early transforms when the script runs before image decoding.
    */
    ...
    function setInitialCover() {
      ...
      // initial top/transform (same as original behavior)
      pic1.style.top = `65%`;
      pic1.style.transform = `translate3d(-50%,-50%,0) scale(${scaleCover})`;
      if (pic1Back) {
        pic1Back.style.top = `65%`;
        pic1Back.style.transform = `translate3d(-50%,-50%,0) scale(${scaleCover}) rotateY(180deg)`;
      }
      // ensure one scroll tick runs to pick up the correct transforms
      latestScrollY = window.scrollY;
      if (!ticking) { ticking = true; requestAnimationFrame(processScroll); }
    }

    // Initial cover is applied only after the image load
    // Handle images loading
    if (pic1) {
      pic1.addEventListener("load", () => {
        // apply initial cover using natural dimensions now that image data is available
        setInitialCover();
      }, { once: true });

      // If already complete and has natural size, apply immediately
      if (pic1.complete && pic1.naturalWidth) {
        // In some cases browser sets complete before script runs, so run in next microtask
        Promise.resolve().then(() => setInitialCover());
      } else if (pic1.complete) {
        // complete but no naturalWidth yet -> short fallback
        setTimeout(() => setInitialCover(), 30);
      }
    } else {
      // no pic1 — still kick off scroll loop
      latestScrollY = window.scrollY;
      if (!ticking) { ticking = true; requestAnimationFrame(processScroll); }
    }
